...
class BatchNormLeNet(EmbedModule):
...
class BatchNormLeNetBuilder(EmbedModuleBuilder):
    def _instantiate_model(self):
        self._model = BatchNormLeNet(self.num_channels, self.num_classes)

    def _process_state_dict(self, d):
        return d

    def _set_trainable(self):
        super(BatchNormLeNetBuilder, self)._set_trainable()
...
builder_class = BatchNormLeNetBuilder
model_args = ()
model_kwargs = dict(
    num_channels=1,
)
